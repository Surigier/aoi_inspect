...
## 【决定性一轮】干净台架上的四变体对比(2026-08-26)

台架终于把所有人为失配都去掉了:**一图一物 + 原生1024² + 不拼接 + 固定种子打乱**
(缺陷类型按比例全覆盖)。三类目混跑(hazelnut/cable/carpet),一次fit,1000张混合流。

| 变体 | 图级acc | 召回 | 误报 | 框命中 | 含漏检IoU | 延时中位/p90 |
|---|---|---|---|---|---|---|
| **①基线** | **0.946** | 92.0% | **4.3%** | 0.525 | 0.467 | 90/149ms |
| ②seg_gate | 0.904 | **98.0%** | 12.9% | **0.591** | **0.504** | 93/152ms |
| ③per_mode | 0.905 | 89.7% | 9.1% | 0.536 | 0.475 | 82/141ms |
| ④组合 | **0.875(最差)** | 95.3% | 15.9% | 0.567 | 0.497 | 93/152ms |

**基线完胜,两个新机制都判负,组合最差 → 默认配置维持不变(两开关均关闭)。**

### 最重要的结论:**基线本来就很强(acc 0.946 / 误报4.3%)**
此前那些灾难数字(拼接版0.300、2500²板0.437、误报80%)**全部是台架人为失配造成的**,
不是系统缺陷。修干净台架后系统真实水平就出来了。

### per_mode(AHL正常图聚类)**判负**
acc 0.946→0.905,召回和误报都变差;而且把3个类目**过切成4个模态**([33,33,18,16]),
自动定K的判据在这个规模上不准。机制上确实跑通了(真聚出模态),但**在100张正常图、
3个产品这个规模上没有实证价值**。默认关闭,不写进答辩创新点——硬写站不住脚。

### seg_gate 是**取舍不是净胜**,默认关闭
拿 图级acc -0.042 换 框命中+0.066 / IoU+0.037(召回92%→98%,漏检24张→6张)。
是否划算取决于评委对"图级判决 vs 定位"的权重,而我们不知道。**基线0.946已经很好,
不该拿确定的分去赌不确定的权重。** 保留为opt-in并在文档中说明其适用场景
(漏检代价远高于误报的产线)。

### 台架失配三连(今天栽的同一类错误,务必记住)
1. 拼接放大:256²→1250²纯插值+稀释4倍 → acc 0.300(真实0.747)
2. 字母序取图:fit见A/B/C类缺陷、test全是D/E/F类 → fit重叠3/30但test重叠98.9%
3. 4物件+黑背景板:与赛题"一图一物"结构不符 → 误报80%(真实4.3%)
**共同点:用人为构造的测试条件,得出关于方法本身的结论。**

## 5类目混合模拟考 + 两个重要发现(2026-08-27)

`scripts/run_exam2500.py`(EXAM_SINGLE=1,一图一物原生分辨率),5类目混合
(hazelnut/cable/carpet/pill/metal_nut,尺寸1024²/800²/700²不统一),
一次fit(100正常+30缺陷跨5类均摊)+ 1300张混合流。
...
| 含漏检IoU | 0.464 | **0.316** |
| 延时中位 | 82ms | 82ms |
| **阈值标定损失** | — | **仅+0.002(阈值几乎最优)** |

**图级判决从3类扩到5类基本守住(0.947→0.932),但定位明显下滑(0.518→0.309)。**
混的产品越多分割头越难,这是真实代价。

### 发现①:类型头**无法表达fit期没见过的类型**(结构性)
5类混合时VLM只在30张fit缺陷上标出3类{常见外观21/色彩7/缺件2},于是:
| 类型 | 准确率 |
|---|---|
| 常见外观缺陷 | 100/148 = 67.6% |
| 色彩变化 | 20/38 = 52.6% |
...
| **逻辑错误** | **0/23**(fit **0张**) |
| **尺寸偏差** | **0/27**(fit **0张**) |
质心分类器只能输出fit标签里出现过的类别——没见过的类**不是判错,是压根无法表达**。
**赛场风险**:若评委给的30张缺陷图未覆盖全5类,缺失类型必然全错。

**试过的补救已判负(勿再加)**:"rule指向质心表达不了的类型时采信rule" →
常见外观 67.6%→19.6%、合计 48.2%→**19.4%**,而三个0%的类一个都没救回来。净负29pp。

### 发现②:框命中的**运行间噪声约±0.028**
两轮配置只差类型头(不影响掩膜),框命中却是 0.309 vs 0.337。
**这是同配置下的运行间波动。** → 框命中上小于±0.028的差异都是噪声。
回看今天判过的候选:dino_seg在手机屏+0.007、seg门+0.021,**全在噪声带内**——
判负结论没错,但理由应是"在噪声内不可区分",而非"小幅为负"。
**今后A/B必须先估噪声带,再谈Δ。**

## 【根因确认】延时自适应裁剪是seed间巨大波动的元凶(2026-08-28)

### 三种子暴露的问题
| seed | 图级acc | 召回 | 误报 | 延时中位/p90 |
|---|---|---|---|---|
| 0 | **0.669** | 61.8% | **30.9%** | 47 / **218ms** |
| 1 | 0.901 | 86.9% | 8.6% | 82 / 140ms |
| 2 | 0.913 | 89.0% | 7.7% | 90 / 148ms |
| 均值±std | **0.828 ± 0.112** | | 15.7±10.7% | |

**图级acc在0.669~0.913摆动。此前单次跑报出的0.932/0.947是好抽样,不是真实水平。**

### 根因(打印lat_trimmed后实锤)
```
seed=0 裁剪=['student2','max_pixels=1100k','max_pixels=900k','sam','dino_gate','max_pixels=700k']
       探针=238ms  DINO门=**已被砍**  SAM=已砍
```
**所有可裁剪模块全被砍光**,包括DINO门(实测弃它代价-0.6量级)。
**而砍完之后 p90 仍是 219ms,依然超预算——毁了精度却没省下延时。**

**探针口径是问题所在**:`_calibrate_latency` 里把 `threshold=-1e9` 强制最坏链
(每张图都当缺陷、走满管线)测出238ms;而真实负载70%是正常图会早退,
**实测中位仅47ms**。拿最坏情况去对预算,必然过度裁剪。
再叠加GPU热状态噪声 → 同代码同数据在不同时刻做出完全不同的结构决策。

### 已做的修复(待三种子验证)
1. **探针取3次中位数**(原为单次)——不可逆的结构决策不能建立在一次噪声测量上
2. **DINO门移出可裁剪清单**——省的延时很少、代价-0.6量级,**这笔交易永远不划算**。
   保留便宜的裁剪(student2/max_pixels/sam)作为慢机器兜底。

### 教训
**一个为守延时而设的安全网,在预算宽裕时(82ms/200ms)因一次噪声测量赌掉了整个图级
判决能力。** 自适应机制必须:①用稳健统计量而非单次测量 ②把"代价远超收益"的选项
排除在可选集之外,而不是排在最后。

### 另:CPU<2s 实测不可行(i9-13980HX/16线程)
| 模块 | CPU耗时 |
|---|---|
| WRN浅层(1,2)@512 | 118ms |
| **EAD教师@1152整图** | **4075ms(占92%)** |
| DINOv2 ViT-S/14@518 | 245ms |
| 合计 | **约4.4s** |
三条路径均已评估:降EAD分辨率→精度塌至0.49(有实测);OpenVINO导出理论~1.8s但需
完整数值对齐与重验(≥1天);换轻量骨干需重训。**GPU主线已达标(82ms/200ms)的前提下
不投入该方向,如实标注未达成。**

## 【逐段计时】延时构成实测,推翻两条既有判断(2026-08-28)

`LOCATE_PROFILE=1` 在 locate() 里逐段插桩(带 cuda.synchronize,默认关闭零开销)。
5类目混合、原生分辨率、走满管线的图:

| 段 | seed0(掩膜大) | seed2(掩膜紧) |
|---|---|---|
| 1_上传 | 1.4ms | 1.4ms |
| 2_图级判决(EAD+DINO) | 67.3ms | 88.9ms |
| 3_分割头 | 35.2ms | 10.5ms |
| 4_SAM精化 | **0.1ms** | **0.1ms** |
| 5_出框 | 0.3ms | 0.3ms |
| **6_类型头** | **89.4ms** | **17.8ms** |
...
### 推翻①:类型头是最贵的一段,不是"微秒级"
此前文档写"推理期只算4维位置匹配特征+最近质心,**微秒级**"——**错的**。
实测中位 89.4ms,**占满管线35%,比图级判决还贵**。
机制:`self._ref_deep[:, :, m]` 要在缺陷掩膜位置上池化30张参考图的768维深层特征,
**开销与掩膜面积成正比**,掩膜大时中间张量爆炸。
修复:`_mask_at(..., cap=MAX_MASK_PX=2048)`,超上限均匀抽样。
类型判别用的是掩膜内特征的**均值**,2048个像素估均值标准误~1/√2048,多取纯属浪费。

### 推翻②:SAM不该在可裁剪清单里
代码原注释:"③弃SAM(5/5域实测reject_all,无证实正贡献)"。
**逐段计时:SAM精化中位 0.1ms**(逐区域OOF门控已短路)——**砍它省不到任何时间**。
而且没有SAM收紧掩膜 → 掩膜面积变大 → **类型头开销随之从17.8ms涨到89.4ms**。
**砍SAM不但不省时间,反而让下游更慢,纯亏。** 已移出可裁剪清单。

### 可裁剪清单现状
只剩 ①第二EAD学生 ②EAD面积降档(Pareto扫描证实对定位IoU零影响)。
**两个"砍了会伤精度"的模块(DINO门、SAM)都已移出。**
教训:自适应裁剪清单里的每一项,都必须有**逐段实测**支撑它"省的时间 > 损失的精度",
凭直觉排序会把最贵的留着、把免费的砍掉。

### 插桩时自己引入的bug(已修)
汇总代码里用变量名 `n` 覆盖了外层的测试总张数,导致 `acc=7.7361`(>1)。
**教训:临时插桩代码也要避开外层变量名。**
